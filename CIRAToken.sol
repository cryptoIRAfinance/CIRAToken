...
// 🌎 Reddit:  https://reddit.com/r/cryptoIRAfinance
...
contract BUSDDividendTracker is DividendPayingToken  {
    ...
    constructor(address _dividentToken) DividendPayingToken("cryptoIRA_Busd_Dividend_Tracker", "cryptoIRA_Busd_Dividend_Tracker",_dividentToken) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 1000 * (10**5); //must hold 1,000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "CryptoIRA_Busd_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "CryptoIRA_Busd_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CryptoIRA contract.");
    }
    ...
    function updateMinimumTokenBalanceForDividends(uint256 _newMinimumBalance) external onlyOwner {
        ...
        minimumTokenBalanceForDividends = _newMinimumBalance * (10**5);
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "CryptoIRA_Busd_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "CryptoIRA_Busd_Dividend_Tracker: Cannot update claimWait to same value");
...
contract MarketingDistributor{
    ...
    IUniswapV2Router02  constant uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);     ///pancake swap

    uint256 public liquidityThreshhold = 1000*(10**5);
    IERC20 public BUSD = IERC20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);   ///busd
    IERC20 public immutable CIRAToken;

    address  public  marketingWallet = 0xaA24b8bFaB73F1b96deDa252A58BF809676A97E6;
    bool public enableLiquify;

    constructor(address _CIRAToken){
        CIRAToken = IERC20(_CIRAToken);
    }
    ...
    function swapAndLiquify() public {
        ...
        uint256 tokenAmount  = CIRAToken.balanceOf(address(this));
        if(tokenAmount>=liquidityThreshhold && enableLiquify){
            tokenAmount = liquidityThreshhold;
            address[] memory path = new address[](2);
            path[0] = address(CIRAToken);
            path[1] = address(BUSD);
            CIRAToken.approve(address(uniswapV2Router), tokenAmount);
...
contract CIRA is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public constant MAX_SUPPLY = 10000000*(10**5);
 
    uint256 public maxBuyTranscationAmount = MAX_SUPPLY.mul(15).div(1000); // 1.5%
    uint256 public maxSellTransactionAmount = MAX_SUPPLY.mul(15).div(1000); // 1.5%
    uint256 public maxWalletBalance =MAX_SUPPLY.mul(15).div(1000); // 1.5%
    uint256 public swapTokensAtAmount = 20 * 10**2 * 10**5;
    ...
    uint256 public _busdDividendRewardsFeeBuy = 7;
    uint256 public _marketingFeeBuy = 1;
    uint256 public _LpFeeBuy = 2;

    uint256 public _busdDividendRewardsFeeSell=7;
    ...
    constructor() ERC20("CryptoIRA", "CIRA") {
        	
    	busdDividendToken = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56; ///////BUSD CONTRACT
    ...
    receive() external payable {
  	...
  	function setMaxBuyTransaction(uint256 _maxTxn) external onlyOwner {
  	    maxBuyTranscationAmount = _maxTxn * (10**5);
  	}
 
  	function setMaxSellTransaction(uint256 _maxTxn) external onlyOwner {
  	    maxSellTransactionAmount = _maxTxn * (10**5);
  	}
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "CryptoIRA: The marketing wallet is already this address");
  	...
  	function setSwapTokensAtAmount(uint256 _swapAmount) external onlyOwner {
  	    swapTokensAtAmount = _swapAmount * (10**5);
  	}
    ...
    function updatebusdDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(busdDividendTracker), "CryptoIRA: The dividend tracker already has that address");
 
        BUSDDividendTracker newbusdDividendTracker = BUSDDividendTracker(payable(newAddress));
 
        require(newbusdDividendTracker.owner() == address(this), "CryptoIRA: The new dividend tracker must be owned by the CryptoIRA token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "CryptoIRA: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "CryptoIRA: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "CryptoIRA: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "CryptoIRA: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "CryptoIRA: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "CryptoIRA: Trading has not started yet");
